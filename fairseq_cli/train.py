...
def train(
    cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, max_epoch
    ...
    step_offset=len(progress)*(epoch_itr.epoch-1)+itr.n
    total_steps=len(progress)*max_epoch
    ...
    for i, samples in enumerate(progress):
        ...
        if hasattr(trainer.model, 'set_progress'):
            trainer.model.set_progress(global_step/total_steps)
...
def validate(
    ...
    for subset in subsets:
        ...
        stats, change_flag = get_valid_stats(cfg, trainer, agg.get_smoothed_values())
        valid_topk_flag[subset] = change_flag
        progress.print(stats, tag=subset, step=trainer.get_num_updates())
        valid_losses.append(stats[cfg.checkpoint.best_checkpoint_metric])

    if getattr(cfg.model, 'dynamic_expert_allocate', False):
        # if has valid subsets, compute avg losses and append on top
        if len(valid_losses) > 1:
            avg_valid_losses = sum(valid_losses) / len(valid_losses)
            valid_losses = ["{:.2f}".format(avg_valid_losses)] + valid_losses

        for subset, change_flag in valid_topk_flag.items():
            if change_flag and (task.top_k_cnt[subset] < cfg.model.moe_expert_count):
                task.top_k_cnt[subset] += 1

        progress.log(task.top_k_cnt, tag="topk", step=trainer.get_num_updates())
        
    if getattr(cfg.model, 'record_token_expert', False):
        display_token_expert(trainer)
    return valid_losses
...
def get_valid_stats(
    ...
    if hasattr(checkpoint_utils.save_checkpoint, "best"):
        ...
        previous_metr = float(checkpoint_utils.save_checkpoint.best)
        current_metr = stats[cfg.checkpoint.best_checkpoint_metric]
        stats[key] = best_function(previous_metr, current_metr) 
        if previous_metr - current_metr < cfg.model.topk_threshold:
    # if "best_ppl" in stats:
            change_flag = True
    return stats, change_flag
